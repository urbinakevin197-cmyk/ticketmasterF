...
useEffect(() => {
  const obtener = async () => {
    const res = await fetch("https://calendar-less-violations-sunshine.trycloudflare.com/api/mis-transferencias", {
      headers: {
...
const aceptarBoleto = async () => {
  ...
  try {
    await fetch(`https://calendar-less-violations-sunshine.trycloudflare.com/api/aceptar-boleto/${t.id}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`
      }
